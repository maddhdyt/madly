...
import ParticipantFormSlideOver from './ParticipantFormSlideOver';
...
export default function ParticipantsIndex({ participants = [] }) {
    ...
    const handleEdit = (p) => { setSelectedParticipant(p); setIsFormOpen(true); };
    const handleCreate = () => { setSelectedParticipant(null); setIsFormOpen(true); };
    const handleDelete = (p) => { setParticipantToDelete(p); setDeleteModalOpen(true); };
    const confirmDelete = () => {
        if (participantToDelete) {
            router.delete(route('accounting.profit-participants.destroy', participantToDelete.id), { onSuccess: () => setDeleteModalOpen(false) });
        }
    };

    return (
        <AccountingLayout title={t('Profit Participants')}>
            <Head title={t('Profit Participants')} />
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between px-8 py-8 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 gap-4 shrink-0">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">
                            {t('Profit Participants')}
                        </h1>
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                            {t('Manage entities that receive profit distributions.')}
                        </p>
                    </div>
                    <div className="flex items-center gap-3 mt-4 sm:mt-0">
                        ...
                        >
                            ...
                            <span className="hidden sm:inline">{t('New Participant')}</span>
                        </button>
                    </div>
                </div>
                ...
                <table className="w-full text-left text-sm text-gray-600 dark:text-gray-300 whitespace-nowrap">
                    <thead className="bg-white dark:bg-gray-900 text-gray-400 dark:text-gray-500 text-[11px] uppercase tracking-widest font-extrabold border-b border-gray-100 dark:border-gray-800 sticky top-0 z-10">
                        <tr>
                                ...
                                <th className="px-8 py-4">{t('Contact')}</th>
                            ...
                            {filteredParticipants.length > 0 ? (
                                filteredParticipants.map((p) => (
                                    <tr key={p.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors group">
                                        ...
                                        <td className="px-8 py-4">
                                            <span className="px-2.5 py-1 rounded-md bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 text-xs font-medium uppercase tracking-wider">
                                                {p.type}
                                            </span>
                                        </td>
                                        <td className="px-8 py-4">
                                            <div className="text-sm text-gray-600 dark:text-gray-400">{p.contact_info || '-'}</div>
                                        </td>
                                        <td className="px-8 py-4">
                                ...
                                <tr>
                                    <td colSpan="5" className="px-8 py-12 text-center text-gray-500 dark:text-gray-400">
                                        <div className="flex flex-col items-center justify-center">
                                            ...
                                            <p className="text-lg font-medium">{t('No participants found')}</p>
                                            <Users className="w-10 h-10 text-gray-200 dark:text-gray-800 mb-2" />
                                            <p className="text-sm font-medium text-gray-900 dark:text-white">{t('No participants found')}</p>
                                            <p className="text-xs text-gray-500">{t('Create your first profit participant to get started.')}</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
            ...
            <ConfirmModal
                ...
                message={t('Are you sure you want to delete this participant? This action cannot be undone.')}
