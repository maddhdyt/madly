                ...
                <table className="w-full text-left text-sm text-gray-600 dark:text-gray-300 whitespace-nowrap">
                    <thead className="bg-white dark:bg-gray-900 text-gray-400 dark:text-gray-500 text-[11px] uppercase tracking-widest font-extrabold border-b border-gray-100 dark:border-gray-800 sticky top-0 z-10">
                        <tr>
                            ...
                            <th className="px-8 py-4">{t('Project')}</th>
                                ...
                                <tr key={revenue.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors group">
                                    ...
                                    <td className="px-8 py-4">
                                        <div className="flex items-center gap-2">
                                            <div className="w-6 h-6 rounded bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
                                                <PiggyBank className="w-3.5 h-3.5 text-gray-900 dark:text-white" />
                                            </div>
                                    ...
                                    <td className="px-8 py-4 text-right">
                                        <div className="flex items-center justify-end gap-1 font-bold text-gray-900 dark:text-white">
                                            <ArrowUpRight className="w-3.5 h-3.5 text-gray-900 dark:text-white" />
                                            {formatIDR(revenue.amount)}
                                        </div>
                                    </td>
                                    <td className="px-8 py-4">
                                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${
                                            revenue.status === 'posted' ? 'bg-gray-100 text-gray-900 dark:bg-gray-800 dark:text-white' :
                                            revenue.status === 'voided' ? 'bg-gray-100 text-gray-900 dark:bg-gray-800 dark:text-white' :
                                    ...
                                    <td className="px-8 py-4 text-right">
                                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button onClick={() => handleEdit(revenue)} className="p-1.5 text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">
                                                <Edit2 className="w-4 h-4" />
                                            </button>
                                            {revenue.status !== 'voided' && (
                                                <button onClick={() => handleDelete(revenue)} className="p-1.5 text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors" title={t('Void Revenue')}>
